...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-6 overflow-hidden rounded-none border-[4px] border-foreground bg-card py-6 text-sm text-card-foreground shadow-[8px_8px_0_0_color-mix(in_oklab,var(--color-foreground)_90%,transparent)] has-[>img:first-child]:pt-0 data-[size=sm]:gap-4 data-[size=sm]:py-4",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("font-sans text-base font-black uppercase tracking-[0.04em]", className)}
      {...props}
    />
  )
}
